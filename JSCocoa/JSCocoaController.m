...
- (id)eval:(NSString *)script            {
    return [self toObject:[self evalJSString:script]];
}
...
- (id)callFunction:(NSString *)name withArguments:(NSArray *)arguments    {
    return [self toObject:[self callJSFunctionNamed:name withArgumentsArray:arguments]];
}

- (BOOL)hasFunction:(NSString *)name     {
    return [self hasJSFunctionNamed:name];
}

- (BOOL)isSyntaxValid:(NSString *)script {
    return [self isSyntaxValid:script error:nil];
}
...
+ (int)JSCocoaPrivateObjectCount        {
    return JSCocoaPrivateObjectCount;
}
...
static JSValueRef jsCocoaObject_callAsFunction(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount, const JSValueRef arguments[], JSValueRef *exception) {
    ...
    if ([privateObject jsValueRef]) {
        ...
        else if ([privateObject.type isEqualToString:@"externalJSValueRef"])      {
